...
void ImageViewer::newImageAccepted()
{
	...
	ui->tabWidget->setCurrentIndex(ui->tabWidget->count() - 1);
	getCurrentViewerWidget()->nakresliOsi(20);
	ui->groupBox->setVisible(true);

}

void ImageViewer::on_kresli_clicked() {
	//qDebug() << "snaha kreslit";
	int typ, N, dieliky;
	if (ui->radioButton->isChecked())
		...
		msgBox.setText(u8"Zvo¾te typ krivky.");
		msgBox.setIcon(QMessageBox::Information);
		msgBox.exec();
		return;
	}
	...
	if (ui->comboBox->currentIndex() == 0) {			
		...
		getCurrentViewerWidget()->nakresliCosinus(N, dieliky, typ);
	}
	
}

void ImageViewer::on_vymaz_clicked() {
